...
| 117 | ~~Gras / souligné dans les zones de texte du dashboard~~ | **Annulé, remplacé par du plus ambitieux** — la version light ne suffira pas pour de vrais articles (Médias & Actus). Éditeur TipTap complet à prévoir avec le CRUD MediaPost (Bloc B, #54) |
...
| 120 | ~~Retirer le champ légende/alt de l'upload hero dans le dashboard~~ | **Annulé** — hero et section "La mission" partagent maintenant un seul champ (`mission_photos`), la légende reste utile pour la section même si elle ne sert à rien pour le hero (`aria-hidden`). Garder la légende sert de filet de sécurité si les 2 photos sont inversées |
...
| 128 | Fusionner l'onglet "Service Civique" dans "Missions" (sidebar) | Un seul onglet "Missions" (renommé, plus "Missions individuelles") affichant tous les types (filtres : type / pays / actif-inactif) au lieu de 2 pages séparées. **Faisable à coût réduit** : `findAllForAdmin()` renvoie déjà tous les types sans filtre côté backend, `MissionsPage.jsx` et `ServiceCiviquePage.jsx` filtrent juste différemment côté front sur le même jeu de données — retirer le filtre fixe, ajouter des filtres UI. ⚠️ Exclure explicitement les 3 missions "ancres" (service civique/groupe jeunes/congé solidaire, non éditables) qui réapparaîtraient sinon dans la liste. Les formulaires d'édition (`MissionFormPage`/`ServiceCiviqueFormPage`) restent séparés pour l'instant (champs différents selon le type) |
| 129 | Supprimer `components/admin/MissionForm.jsx` | Code mort confirmé (aucune référence nulle part), repéré pendant le chantier palette — à supprimer après vérification à tête reposée, pas fait dans la foulée |
| 130 | Pré-remplir `EMPTY_FORM` — tarifs standards + inclus/non-inclus | Décidé le 18/09, jamais implémenté ni noté à l'époque. 4 lignes de tarifs standards (10j/1175€, 2sem/1500€, 3sem/2000€, 4sem/2500€) + textes inclus/non-inclus standards, tous modifiables/supprimables à la création. N'affecte que la création, aucun impact sur les missions déjà saisies |
