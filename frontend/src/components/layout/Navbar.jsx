...
// Missions : dynamique depuis l'API (type volontariat_individuel)
// ════════════════════════════════════════════════════════════════
...
// Liens du dropdown "À propos"
const APROPOS = [
  { label: "Notre association", href: "/a-propos" },
  { label: "Notre équipe", href: "/equipe" },
]

// Liens statiques "S'engager autrement" — colonne droite du dropdown missions
const ENGAGEMENTS = [
...
function Navbar() {
  ...
  // ── Données dynamiques ──
  const [missionsNav, setMissionsNav] = useState([])

  // ── Timers de fermeture des dropdowns au survol ──
  ...
  // ── Fetch des missions volontariat individuel au montage ──
  useEffect(() => {
    fetchMissions({ type: 'volontariat_individuel' })
      .then(data => setMissionsNav(data ?? []))
      .catch(() => {}) // silencieux — la navbar ne doit jamais planter
  }, [])

  // ── Fermeture complète du menu mobile ──
  ...
  return (
    <>
      {/* ════════════════════════════════════════════════════════════════
          BARRE PRINCIPALE
          Note : px volontairement différent de padding-x
      ...
      <nav className="w-full flex items-center justify-between px-4 lg:px-8 xl:px-16 h-20 bg-transparent absolute top-0 left-0 z-10">
        ...
        <div className="hidden lg:flex items-center gap-sm xl:gap-lg">
          ...
          {/* ── DROPDOWN "NOS MISSIONS" — 2 colonnes ── */}
          <div
          ...
          >
            ...
            {missionsOpen && (
              <div className="absolute top-full left-0 mt-2 bg-primary rounded-xl shadow-lg py-4 z-20 flex min-w-[420px]">

                {/* Colonne gauche — destinations dynamiques */}
                <div className="flex flex-col flex-1 border-r border-surface/10 pr-2">
                  <p className="px-5 py-1 text-surface/40 text-xs uppercase tracking-wider mb-1">
                    Destinations
                  </p>
                  <Link
                    to="/missions"
                    onClick={() => setMissionsOpen(false)}
                    className="block px-5 py-2 link-nav text-surface hover:text-accent"
                  >
                    Toutes les missions
                  </Link>
                  {missionsNav.map(m => (
                ...
                {/* Colonne droite — s'engager autrement (statique) */}
        ...
        <div className="flex items-center gap-sm">
          ...
          */}
          ...
          <button
            ...
            onClick={() => {
              if (mobileOpen) { setMobileMissionsOpen(false); setMobileAproposOpen(false) }
          ...
          <div>
            ...
            {mobileMissionsOpen && (
              <div className="flex flex-col pl-4 py-2 gap-xs">
                ...
                <p className="text-surface/40 text-xs uppercase tracking-wider pt-2 pb-1">Destinations</p>
                ...
                {/* S'engager autrement */}
                <p className="text-surface/40 text-xs uppercase tracking-wider pt-3 pb-1">S'engager autrement</p>
                {ENGAGEMENTS.map(e => (
                  <Link
                    key={e.filter}
                    to={`/missions?filter=${e.filter}`}
